...
function foundationpress_pagination() {
	...
	$paginate_links = paginate_links( array(
		...
		'mid_size' => 5,
		'prev_next' => true,
	    'prev_text' => __( '&laquo;', 'adagio-foundation-starter' ),
	    'next_text' => __( '&raquo;', 'adagio-foundation-starter' ),
		'type' => 'list',
	) );
...
function foundationpress_menu_fallback() {
	...
	// Translators 1: Link to Menus, 2: Link to Customize.
		printf( __( 'Please assign a menu to the primary menu location under %1$s or %2$s the design.', 'adagio-foundation-starter' ),
			sprintf(  __( '<a href="%s">Menus</a>', 'adagio-foundation-starter' ),
				get_admin_url( get_current_blog_id(), 'nav-menus.php' )
			),
			sprintf(  __( '<a href="%s">Customize</a>', 'adagio-foundation-starter' ),
...
class Adagio_Foundation_Starter_Comments extends Walker_Comment{
	...
	function __construct() { ?>
        ...
        <h3><?php comments_number( __( 'No Responses to', 'adagio-foundation-starter' ), __( 'One Response to', 'adagio-foundation-starter' ), __( '% Responses to', 'adagio-foundation-starter' ) ); ?> &#8220;<?php the_title(); ?>&#8221;</h3>
	...
	function start_el( &$output, $comment, $depth = 0, $args = array(), $id = 0 ) {
		...
		<header class="comment-author">	
			...
			<?php printf( __( '<cite class="fn">%s</cite>', 'adagio-foundation-starter' ), get_comment_author_link() ) ?>
			<time datetime="<?php echo comment_date( 'c' ) ?>"><a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ) ?>"><?php printf( __( '%1$s', 'adagio-foundation-starter' ), get_comment_date(),  get_comment_time() ) ?></a></time>
